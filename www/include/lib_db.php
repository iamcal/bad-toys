<?
	...
	#	log_notice('db', "DB-$cluster_key: Connect", $end-$start);
		...
		if (!$GLOBALS['db_conns'][$cluster_key] || $GLOBALS['cfg']['admin_flags_no_db']){
			...
			die("Connection to database cluster '$cluster_key' failed");
		}
	...
	function _db_insert($tbl, $hash, $cluster, $k=null){
		...
		$flags = $GLOBALS['db_insert_ignore'] ? ' IGNORE' : '';

		return _db_write("INSERT{$flags} INTO $tbl (`".implode('`,`',$fields)."`) VALUES ('".implode("','",$hash)."')", $cluster, $k);
	}
